function [R_exch_data, Q, Qe] = ...
            R_exch_23_e(Ms, n_M1, n_M2, n_M3, n_M4, n_M5, T, reaction, n0)
...
%                     + reshape(E_t{5}, 1, 1, 1, 1, []) - E_t{1}' - E_t{2};
    % dE for Q + rotational energy Q
dE_Q = reshape(E_t{3}, 1, 1, []) + reshape(E_t{4}, 1, 1, 1, []) ...
    + reshape(E_t{5}, 1, 1, 1, 1, []) - E_t{1}' ...
                                    - max(E_t{2}, reaction.E_th) + k * T;
Q = sum(R_exch_data(i_sum{1}, i_sum{2}, i_sum{3}, i_sum{4}, i_sum{5}) ...
                                                        .* dE_Q, 'all');
Qe = sum(R_exch_data(i_sum{1}, i_sum{2}, i_sum{3}, i_sum{4}, i_sum{5}) .* ...
            max(reaction.E_th - E_t{2}, 0), 'all');
end