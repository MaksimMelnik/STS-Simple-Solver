function [kd, dE_fb] = k_exch_Savelev(T, kd_eq, reaction, Ps_r, Ps_p, U)
% Exchange rate coefficient using the Savelev model [1].
...
% [1] E Kustova and A Savelev 2021 J. Phys.: Conf. Ser. 1959 012033
% 29.05.2024 Maksim Melnik
...
dE = (ET + Ep) - Er;
exp_dE_Heav = exp(- dE .* heaviside(dE) / k * (1 / T + 1 / U));
expdE_Pneq = exp_dE_Heav .* n_eq;
sum_expdE_Pneq = sum(expdE_Pneq, 'all');
B = kd_eq / sum_expdE_Pneq;
kd = B * exp_dE_Heav;       % the coefficient
