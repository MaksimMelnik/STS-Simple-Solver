function out = R_exch_M1_M2_M3_M4(M1, M2, M3, M4, n_M1, n_M2, n_M3, n_M4, T, coll)
...
exp_M1 = exp(-(M1.ev_0(1) + M1.ev_i{1})/(V_K*T));
Zv_M1 = sum(exp_M1);
...
% разница энергий, K
dE_M1 = coll.ArrE - repmat((M1.ev_0(1) + M1.ev_i{1})'/V_K,1,M3.num_vibr_levels(1)) + ...
...
Kdr = (M1.mass*M2.mass/(M3.mass*M4.mass))^1.5*Z_rot_M1/Z_rot_M3*...
    exp((repmat(M3.ev_i{1}+M3.ev_0,M1.num_vibr_levels(1),1)-repmat((M1.ev_i{1}+M1.ev_0)',1,M3.num_vibr_levels(1)))/(V_K*T))*...
