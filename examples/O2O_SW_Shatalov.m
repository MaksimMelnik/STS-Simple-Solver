...
struct_dim = zeros(5, 2, 4);
out = struct("res", struct_dim, "time_mus", struct_dim, ...
                                    "Tv", struct_dim, "T", struct_dim);
...
for i_ini = 1 % [1 2 3 4 5]         % choosing desired initial coonditions
 ...
 p0 = init_c_Shatalov(i_ini, 1)*Torr; % Pa;    initial pressure
 n0 = p0/k/T0;                        % m-3; characteristic number density
 rho0 = n0 * O2.mass;                 % kg/m3; initial gas density
 sigma0 = pi*O2.diameter^2;
 Delta = 1 / sqrt(2) / n0 / sigma0;   % characteristic length
    % conservation laws to evaluate macroparameters behind the SW
 ...
 for i_U = 4 % [2 3 4]   % choosing desired U dissociation parameter model
  ...
  for i_vibr = 2 % [1 2]  % choosing vibrational energy exchange model
   ...
   Reacs_keys = {'Diss', 'VT', 'VV'};       % included processes
   reacs_val = {Diss, model_VT, model_VT};  % models of processes above
        % inicialization of kinetics structure
   ...
   e_i = zeros(1, sum(O2.num_vibr_levels(1:O2.num_elex_levels)));
   for ind_e = 1:O2.num_elex_levels
    e_i(1+sum(O2.num_vibr_levels(1:ind_e-1)):...
                    sum(O2.num_vibr_levels(1:ind_e))) = ...
                        O2.ev_i{ind_e} + O2.ev_0(ind_e) + O2.e_E(ind_e);
   end
   En0 = n0*e_i*n/n1 + n0*k*T0 + 1.5*n0*k*T0 + n0*O2.form_e;    % energy
