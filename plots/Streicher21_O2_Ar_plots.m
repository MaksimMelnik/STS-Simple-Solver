...
% p5=plot(data_minor(i_vibr_minor,2,var,rel).time, ...
%     data_minor(i_vibr_minor,2,var,rel).nO2*1e3,'r--', 'LineWidth', ...
%     1.5, 'DisplayName', "\it n_{\rm O_2} - U=D/6k minor" );
% p6=plot(data_minor(i_vibr_minor,3,var,rel).time, ...
%     data_minor(i_vibr_minor,3,var,rel).nO2*1e3,'b--', 'LineWidth', ...
%     1.5, 'DisplayName', "\it n_{\rm O_2} - U=3T minor" );
% p7=plot(data_minor(i_vibr_minor,4,var,rel).time, ...
%     data_minor(i_vibr_minor,4,var,rel).nO2*1e3, 'm--','LineWidth', ...
...
i_vibr_bench = 1;
i_U_bench = 3;
s_bench = spline(data_main(i_vibr_bench, i_U_bench, var, 2).time, ...
    data_main(i_vibr_bench, i_U_bench, var, 2).Tv, ...
    min(time_Tv_exp):0.1:max(time_Tv_exp)); %SSH 3T
