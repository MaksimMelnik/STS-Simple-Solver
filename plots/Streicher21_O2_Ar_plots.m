...
p2=plot(data_main(i_vibr,2,var,rel).time, ...
    data_main(i_vibr,2,var,rel).nm_n*1e3,'r-', 'LineWidth', ...
    1.5, 'DisplayName', "\it n_{\rm O_2} - U=D/6k " );
p3=plot(data_main(i_vibr,3,var,rel).time, ...
    data_main(i_vibr,3,var,rel).nm_n*1e3,'b-', 'LineWidth', ...
    1.5, 'DisplayName', "\it n_{\rm O_2} - U=3T " );
p4=plot(data_main(i_vibr,4,var,rel).time, ...
    data_main(i_vibr,4,var,rel).nm_n*1e3, 'm-','LineWidth', ...
...
ylim([20 max(data_main(i_vibr,2,var,rel).nm_n*1e3) + 4]);
hold off
grid minor
end
