 ...
 for ind_exch = 1:length(exch_reactions)
  ...
  switch length(reaction.particles)
   ...
   case 5
    ...
    R_exch_data(indM1) = R_exch_data(indM1) + sum(R_exch_temp, [2 3 4 5]);
    R_exch_data(indM2) = R_exch_data(indM2) + sum(R_exch_temp,[1 3 4 5])';
    R_exch_data(indM3) = R_exch_data(indM3) ...
                            - reshape(sum(R_exch_temp, [1 2 4 5]), [], 1);
    R_exch_data(indM4) = R_exch_data(indM4) ...
