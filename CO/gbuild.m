...
axesH=axes;
plot(...
res_all(i_ini,1,i_U).temp(:,end),res_all(i_ini,1,i_U).temp(:,1+num_C),...
    'linewidth', 1.5)
hold on
...res_all(i_ini,2,i_U).temp(:,end),
            ...res_all(i_ini,2,i_U).temp(:,1+num_C),...
plot(res_all(i_ini, 3, i_U).temp(:, end),...
                res_all(i_ini, 3, i_U).temp(:, 1+num_C),...
    'color', [255 170 0]/255, 'linewidth', 1.5)
