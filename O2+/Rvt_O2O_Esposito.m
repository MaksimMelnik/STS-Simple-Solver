function out=Rvt_O2O_Esposito(n, na, T, O2)
% k i->i-1 by Esposito 2008
% 23.02.2021
cijk(1, :, :)=[
...
for i=1:41
    ...
    if i-di>=0
     if v-en(i-di+1,2)<31
        ...
        k = 1.3807e-23;
        kbf=krate*exp(-(O2.e_i(1,i+1)-O2.e_i(1,i-di+1))/k/T);
%         if i==1
%             disp('krate Esposito')
%             disp(krate)
%             disp(kbf)
%             disp(O2.e_i(1,i+1)-O2.e_i(1,i-di+1))
%         end

%         gg=a(1)+a(2)./log(T);%+a(3)*log(T);
%         disp(a(3))
%         disp(krate(1))
        R=(krate*n(i+1)-kbf*n(i+1-di))*na;
%         disp(i)
%         disp(out(i+1))
        out(i+1)=out(i+1)-R;
        out(i+1-di)=out(i+1-di)+R;
%         disp(num2str([i i-di krate kbf v v-dv]))
