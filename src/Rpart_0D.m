...
for ind = 1:kinetics.num_Ps
    if kinetics.Ps{ind}.fr_deg_c > 3
        ...
        E_term = E_term + sum((R(kinetics.index{ind}))' .* (e_i + kinetics.Ps{ind}.form_e));
        Rci_term = Rci_term + 2.5 * sum(R(kinetics.index{ind}));
        nm = nm + sum(y(kinetics.index{ind}));
    else
        
        E_term = E_term + R(kinetics.index{ind}) .* (kinetics.Ps{ind}.form_e + kinetics.Ps{ind}.e_E(1:kinetics.Ps{ind}.num_elex_levels));
        Rci_term = Rci_term + 1.5 * sum(R(kinetics.index{ind}));
        na = na + sum(y(kinetics.index{ind}));
    end
end
M(kinetics.num_eq + 1) = -(T_DN * Rci_term + E_term / k / kinetics.T0) / (2.5 * nm + 1.5 * na);
out = M;
end
