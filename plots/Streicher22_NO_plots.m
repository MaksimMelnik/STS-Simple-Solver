...
load('../data/NO Streicher experiment/NO_behindRSW_output.mat');
...
for var=7
%testcases     
%var: %1 - 2-02 T=3560 K, P=0.561 atm;  2 - 2-14 T=5460 K, P=0.325 atm;
%3 - 2-32 T=7070 K, P=0.119 atm; 4 - 2-38 T=8730 K, P=0.137 atm;
%5 - 1-01 T=2360 K, P=0.757 atm; 6 - 1-04 T=3470 K, P=0.584 atm;
%7 - 1-08 T=5580 K, P=0.252 atm; 8 - 1-12 T=7090 K, P=0.274 atm;
%9 - 04-02 T=2180 K, P=0.952 atm; 10 - 04-11 T=3470 K, P=0.668 atm;
%11 - 04-22 T=5650 K, P=0.545 atm;

i_vibr=2; %model of vibrational enegry exchange 1 - SSH, 2 - FHO, 3 - FHO-FR
...
%     1.5, 'DisplayName', "\it T - U=D/6k, FHO" );
% p2=plot(data_main(i_vibr,3,var,rel).time, ...
%     data_main(i_vibr,3,var,rel).T,'b--', 'LineWidth', ...
%     1.5, 'DisplayName', "\it T - U=3T, FHO" );
% p3=plot(data_main(i_vibr,4,var,rel).time, ...
%     data_main(i_vibr,4,var,rel).T, 'm--','LineWidth', ...
%     1.5, 'DisplayName', "\it T - U=\infty, FHO" );
% p5=plot(data_main(i_vibr+1,2,var,rel).time, ...
%     data_main(i_vibr+1,2,var,rel).T,'r:', 'LineWidth', ...
%     1.5, 'DisplayName', "\it T - U=D/6k, FHO-FR" );
% p6=plot(data_main(i_vibr+1,3,var,rel).time, ...
%     data_main(i_vibr+1,3,var,rel).T,'b:', 'LineWidth', ...
%     1.5, 'DisplayName', "\it T - U=3T, FHO-FR" );
% p7=plot(data_main(i_vibr+1,4,var,rel).time, ...
%     data_main(i_vibr+1,4,var,rel).T, 'm:','LineWidth', ...
%     1.5, 'DisplayName', "\it T - U=\infty, FHO-FR" );
...
% legend([p1 p5 p2 p6 p3 p7 p4],'Location','best');
% xlim([-10 tlim_T]);
...
%     1.5, 'DisplayName', "{\it T}_v^{NO} - \it U=D/6k, FHO" );
% plot(data_main(i_vibr+1,2,var,rel).time, ...
%     data_main(i_vibr+1,2,var,rel).TvNO,'r:', 'LineWidth', ...
%     1.5, 'DisplayName', "{\it T}_v^{NO} - \it U=D/6k, FHO-FR" );
% plot(data_main(i_vibr,3,var,rel).time, ...
%     data_main(i_vibr,3,var,rel).TvNO,'b-', 'LineWidth', ...
%     1.5, 'DisplayName', "{\it T}_v^{NO} - \it U=3T, FHO" );
% plot(data_main(i_vibr+1,3,var,rel).time, ...
%     data_main(i_vibr+1,3,var,rel).TvNO,'b:', 'LineWidth', ...
%     1.5, 'DisplayName', "{\it T}_v^{NO} - \it U=3T, FHO-FR" );
% plot(data_main(i_vibr,4,var,rel).time, ...
%     data_main(i_vibr,4,var,rel).TvNO, 'm-','LineWidth', ...
%     1.5, 'DisplayName', "{\it T}_v^{NO} - \it U=\infty, FHO" );
% plot(data_main(i_vibr+1,4,var,rel).time, ...
%     data_main(i_vibr+1,4,var,rel).TvNO, 'm:','LineWidth', ...
%     1.5, 'DisplayName', "{\it T}_v^{NO} - \it U=\infty, FHO-FR" );
...
%     1.5, 'DisplayName', "\it n_{\rm NO} - U=D/6k, FHO" );
% p3=plot(data_main(i_vibr,3,var,rel).time, ...
%     data_main(i_vibr,3,var,rel).nNO*1e3,'b-', 'LineWidth', ...
%     1.5, 'DisplayName', "\it n_{\rm NO} - U=3T, FHO" );
% p4=plot(data_main(i_vibr,4,var,rel).time, ...
%     data_main(i_vibr,4,var,rel).nNO*1e3, 'm-','LineWidth', ...
%     1.5, 'DisplayName', "\it n_{\rm NO} - U=\infty, FHO" );
% Only FHO-FR ---------------
% p5=plot(data_main(i_vibr+1,2,var,rel).time, ...
%     data_main(i_vibr+1,2,var,rel).nNO*1e3,'r--', 'LineWidth', ...
%     1.5, 'DisplayName', "\it n_{\rm NO} - U=D/6k" );
% p6=plot(data_main(i_vibr+1,3,var,rel).time, ...
%     data_main(i_vibr+1,3,var,rel).nNO*1e3,'b--', 'LineWidth', ...
%     1.5, 'DisplayName', "\it n_{\rm NO} - U=3T" );
% p7=plot(data_main(i_vibr+1,4,var,rel).time, ...
%     data_main(i_vibr+1,4,var,rel).nNO*1e3, 'm--','LineWidth', ...
%     1.5, 'DisplayName', "\it n_{\rm NO} - U=\infty" );
% ---------------------------
% Only U = \infty
p5=plot(data_main(1,4,var,rel).time, ...
    data_main(1,4,var,rel).nNO*1e3,'g-', 'LineWidth', ...
    1.5, 'DisplayName', "\it n_{\rm NO} - U=\infty, SSH" );
p6=plot(data_main(2,4,var,rel).time, ...
    data_main(2,4,var,rel).nNO*1e3,'b-.', 'LineWidth', ...
    1.5, 'DisplayName', "\it n_{\rm NO} - U=\infty, FHO" );
p7=plot(data_main(3,4,var,rel).time, ...
    data_main(i_vibr+1,4,var,rel).nNO*1e3, 'm--','LineWidth', ...
    1.5, 'DisplayName', "\it n_{\rm NO} - U=\infty, FHO-FR" );
% ---------------------------
...
legend([p1 p5 p6 p7],'Location','best');
xlabel("\it t\rm, \mus");
...
%     1.5, 'DisplayName', "{\it n}^0_{NO} - \it U=D/6k, FHO" );
% p3=plot(data_main(i_vibr,3,var,rel).time, ...
%     data_main(i_vibr,3,var,rel).ni_NO(:,1)/Na*1e3,'b--', 'LineWidth', ...
%     1.5, 'DisplayName', "{\it n}^0_{NO} - \it U=3T, FHO" );
% p4=plot(data_main(i_vibr,4,var,rel).time, ...
%     data_main(i_vibr,4,var,rel).ni_NO(:,1)/Na*1e3, 'm--','LineWidth', ...
%     1.5, 'DisplayName', "{\it n}^0_{NO} - \it U=\infty, FHO" );
% p5=plot(data_main(i_vibr,2,var,rel).time, ...
%     data_main(i_vibr,2,var,rel).ni_NO(:,1)/Na*1e3,'r:', 'LineWidth', ...
%     1.5, 'DisplayName', "{\it n}^0_{NO} - \it U=D/6k, FHO-FR" );
% p6=plot(data_main(i_vibr,3,var,rel).time, ...
%     data_main(i_vibr,3,var,rel).ni_NO(:,1)/Na*1e3,'b:', 'LineWidth', ...
%     1.5, 'DisplayName', "{\it n}^0_{NO} - \it U=3T, FHO-FR" );
% p7=plot(data_main(i_vibr,4,var,rel).time, ...
%     data_main(i_vibr,4,var,rel).ni_NO(:,1)/Na*1e3, 'm:','LineWidth', ...
%     1.5, 'DisplayName', "{\it n}^0_{NO} - \it U=\infty, FHO-FR" );
% errorbar(time_n0_err, n0_err, err_n0, 'sqk', 'MarkerFaceColor', ...
%     'k','MarkerSize',1, 'LineWidth', 1);
% xlim([-10 tlim_n]);
% legend([p1 p2 p5 p3 p6 p4 p7],'Location','best');
% xlabel("\it t\rm, \mus");
...
p2=plot(data_minor(i_vibr,2,var,rel).time, g1,'r-', 'LineWidth', ...
    1.5, 'DisplayName', "{\it n}_{NO} -\it U=D/6k main" );
p3=plot(data_minor(i_vibr,3,var,rel).time, g2,'b-', 'LineWidth', ...
    1.5, 'DisplayName', "{\it n}_{NO} -\it U=3T main" );
p4=plot(data_minor(i_vibr,4,var,rel).time, g3, 'm-','LineWidth', ...
    1.5, 'DisplayName', "{\it n}_{NO} -\it U=\infty main" );
...
[~,j33]=min(abs(data_minor(i_vibr, i_U, var, rel).time...
    - tlim_n));
if VDF_NO_plot
n_bolz1=density_f_exc(data_main(i_vibr, i_U, var, rel).TvNO(1),...
    data_main(i_vibr, i_U, var, rel).nNO(1)*Na, NO);
n_bolz2=density_f_exc(data_main(i_vibr, i_U, var, rel).TvNO(j1),...
    data_main(i_vibr, i_U, var, rel).nNO(j1)*Na, NO);
n_bolz3=density_f_exc(data_main(i_vibr, i_U, var, rel).TvNO(j2),...
    data_main(i_vibr, i_U, var, rel).nNO(j2)*Na, NO);
n_bolz4=density_f_exc(data_main(i_vibr, i_U, var, rel).TvNO(j3),...
    data_main(i_vibr, i_U, var, rel).nNO(j3)*Na, NO);
ylim_low=double(min([min(n_bolz1./nSUMmain(1)),...
    min(n_bolz2./nSUMmain(j1)),...
    min(n_bolz3./nSUMmain(j2)),...
    min(n_bolz4./nSUMmain(j3)),...
    min(data_main(i_vibr, i_U, var, rel).ni_NO(4, :)./...
    nSUMmain(4)), min(data_main(i_vibr, i_U, var, rel).ni_NO(j1, :)./...
    nSUMmain(j1)), min(data_main(i_vibr, i_U, var, rel).ni_NO(j2, :)./...
    nSUMmain(j2)), min(data_main(i_vibr, i_U, var, rel).ni_NO(j3, :)./...
    nSUMmain(j3)), min(data_minor(i_vibr, i_U, var, rel).ni_NO(4, :)./...
    nSUMminor(4)), min(data_minor(i_vibr, i_U, var, rel).ni_NO(j11, :)./...
...
nexttile %t=0 
semilogy(0:NO.num_vibr_levels(1)-1, n_bolz1./nSUMmain(1), 'k-',...
...
semilogy(0:NO.num_vibr_levels(1)-1,...
    ...
    nSUMminor(1) ,'-','color',[0 0.6 0],'LineWidth',2.0,...
    "DisplayName","Without exch. react.");
semilogy(0:NO.num_vibr_levels(1)-1, ...
    data_main(i_vibr, i_U, var, rel).ni_NO(1, :)./...
    nSUMmain(1) , 'm-','LineWidth',2.0, "DisplayName",...
...
ylim([ylim_low 1])
text(3,ylim_low*1e2,"{\it t} = 0 \mus," + " {\it T}_v^{NO}({\itt}) = "...
...
nexttile %t=tlim/3
semilogy(0:NO.num_vibr_levels(1)-1, n_bolz2./nSUMmain(j1), 'k-', ...
    'LineWidth',2.0);
set(gca,'FontSize',12);
set(gca, 'FontName', 'Times New Roman');
hold on;
semilogy(0:NO.num_vibr_levels(1)-1,...
    ...
    nSUMminor(j11) ,'-','color',[0 0.6 0],'LineWidth',2.0);
semilogy(0:NO.num_vibr_levels(1)-1,...
    data_main(i_vibr, 2, var, rel).ni_NO(j1, :)./...
    nSUMmain(j1) , 'm-','LineWidth',2.0);
xlabel('NO vibr. level,\it i');
ylabel('\it n_{\rm NO\iti}/\itn');
xlim([0 NO.num_vibr_levels(1)-1]);
ylim([ylim_low 1])
...
nexttile %t=2*tlim/3
semilogy(0:NO.num_vibr_levels(1)-1, n_bolz3./nSUMmain(j2), ...
    'k-', 'LineWidth',2.0);
set(gca,'FontSize',12);
set(gca, 'FontName', 'Times New Roman');
hold on;
semilogy(0:NO.num_vibr_levels(1)-1, ...
    ...
    nSUMminor(j22) ,'-','color',[0 0.6 0],'LineWidth',2.0);
semilogy(0:NO.num_vibr_levels(1)-1, ...
    data_main(i_vibr, i_U, var, rel).ni_NO(j2, :)./ ...
    nSUMmain(j2) , 'm-','LineWidth',2.0);
xlabel('NO vibr. level,\it i');
ylabel('\it n_{\rm NO\iti}/\itn');
xlim([0 NO.num_vibr_levels(1)-1]);
ylim([ylim_low 1])
