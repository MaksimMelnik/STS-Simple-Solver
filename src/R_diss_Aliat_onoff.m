...
for i=1:M.num_elex_levels       % вот тут энергию e_i от нуля или от e_0?
 ZvibT=sum(exp(-M.ev_i{i}(1:M.num_vibr_levels(i))/(V_K*T)));
 ZvibU=sum(exp(M.ev_i{i}(1:M.num_vibr_levels(i))/(V_K*U(i))));
 if ind_Aliat=="Aliat"
  ...
  V(1+sum(M.num_vibr_levels(1:i-1)):sum(M.num_vibr_levels(1:i)))=...
                exp((M.ev_i{i}(1:M.num_vibr_levels(i))+M.e_E(i))/V_K*(1/T+1/U(i)))*ZvibT/ZvibU;
  divsum=1;
 end
